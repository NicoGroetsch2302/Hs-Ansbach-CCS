...
Ein Notebook besteht damit nur noch aus Konfiguration und Aufrufen:
    ...
    from tep.eigen import run_spectra, aggregate, export, plot_all
...
from .aggregate import aggregate, export, load, value_columns
from .config import SpectrumConfig
from .data import faultfree_by_run, load_train, merge_faults
from .plots import (plot_all, plot_bars, plot_cv, plot_dyca_mn_estimate,
                    plot_means, plot_scalar, plot_stds, transform)
from .spectra import SPECTRA, Spectrum, get, register, run_spectra

__all__ = [
    "SpectrumConfig",
    "load_train", "merge_faults", "faultfree_by_run", "fit_scaler",
    "run_spectra", "SPECTRA", "Spectrum", "register", "get",
    "aggregate", "export", "load", "value_columns",
    "plot_all", "plot_means", "plot_stds", "plot_cv", "plot_bars",
...
def fit_scaler(cfg: SpectrumConfig, verbose: bool = True):
    ...
    Gebraucht wird er bei scaling_mode="scaler" und immer bei LDA (das
    Verfahren erzwingt die spaltenweise Standardisierung).
    """
    needed = (cfg.scaling_mode == "scaler"
              or get(cfg.method).forced_scaling == "scaler")
    if not needed:
        return None
